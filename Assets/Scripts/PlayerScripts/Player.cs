...
public class Player : MonoBehaviour {
    /** the GameMaster script keeps track of player position and rot if needed
     *  Vector3 GameMaster.lastPlayerPos
     *  Quaternion GameMaster.LastPlayerRot
     * 
     **/
    ...
    // Points effects
    public EffectGainScore points10;
    
    // NEW non physics
    Vector3 velocity = Vector3.zero;
    Vector3 upVel = Vector3.zero;
    float floatUpSpeed = 0.2f;
	...
	void Update () {
        // Keep track of player position and rotation
...
//				SwimSound();
    ...
    // When player collides with anything, do it here. 
    void OnTriggerEnter2D(Collider2D collider)
    {
        ...
        {
            DieSound();
            gravity = new Vector3(0, floatUpSpeed, 0);
            ...
            //*********** when the fish dies, it floats up slowly (SOMETIMES fish falls like dead wait, especially if doing flip 
            // when killed, though I like it like that ads variety[EH]
            //upVel.y = floatUpSpeed * Time.deltaTime;
            //transform.position += upVel;

        }

        if (collider.tag == "Pickup +10")
        {
            Debug.Log("Player triggered a pickup");
            Instantiate(points10, GameMaster.lastPlayerPos, Quaternion.identity);
        }
